...
export class ACLService implements CanActivate {
  ...
  checkRoute(routeString: string, params?: any): boolean {
    ...
    if (
      route.routeDef.allowRoles &&
      user.roles &&
      route.routeDef.allowRoles.some(role => user.roles.indexOf(role) !== -1)
    )
      return true;
    ...
    return !!(
      route.routeDef.allowCheck && route.routeDef.allowCheck(user, route.params)
    );
  }
  ...
  findRoute(searchRoute: string): any {
    ...
    const result = this.routes.some(routeDef => {
      ...
      let paramNames = paramNamesMatches
        ? paramNamesMatches.map(item => item.substr(1))
        : [];

      // include subpages
      // /admin => enabled is /admin, /admin/, /admin/* not /administrator
      const routeWithoutSlash =
        routeDef.route[routeDef.route.length - 1] === "/"
          ? routeDef.route.slice(0, routeDef.route.length - 1)
          : routeDef.route;
    ...
    else if (this.configService.config.acl.default)
      return {
