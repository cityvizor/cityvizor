...
export class EventDetailComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (
      (changes.profileId || changes.eventId || changes.year) &&
  ...
  async loadEvent(profileId: number, eventId: number, year: number) {
    ...
    this.maxHistoryAmount = this.history.reduce(
      (acc, cur) =>
        Math.max(
          acc,
    ...
    if (this.event.payments) {
      ...
      this.event.payments.forEach(payment => {
        let counterparty = payment.counterpartyId
          ? counterpartyIndex[payment.counterpartyId]
          : noCounterparty;
        if (!counterparty) {
