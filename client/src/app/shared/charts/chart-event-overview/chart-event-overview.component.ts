...
export class ChartEventOverviewComponent {
  ...
  set event(event: BudgetEvent) {
    ...
    this.amount = overdraft
      ? (event[keys.amount] - overdraft) / event[keys.amount]
      : event[keys.amount] / event[keys.budgetAmount];

    const paymentsAmount = event.payments
      ? event.payments.reduce((acc, cur) => acc + cur[keys.amount], 0)
      : 0;
