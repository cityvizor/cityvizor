...
router.post("/profiles/:profile/payments",
...
router.patch("/profiles/:profile/payments",
    upload.fields([{name: "payments"}]),
    schema.validate(importAccountingSchema),
    acl("profile-accounting", "write"),
    async (req, res, _) => {
...
router.post("/profiles/:profile/events",
...
router.patch("/profiles/:profile/events",
    upload.fields([{name: "events"}]),
    schema.validate(importAccountingSchema),
    acl("profile-accounting", "write"),
    async (req, res, _) => {
...
router.post("/profiles/:profile/data",
...
router.patch("/profiles/:profile/data",
    upload.fields([{name: "data"}]),
    schema.validate(importAccountingSchema),
    acl("profile-accounting", "write"),
    async (req, res, _) => {
...
router.patch("/profiles/:profile/accounting",
...
async function createWorkerTask(req, res, type: FileType, isAppend: boolean) {
    ...
    if (!req.files || !req.files[type.toString()]) return res.status(400).send("Missing data file or zip file");
    ...
    if (!year) {
        const yearInsert = await db<YearRecord>("app.years").insert({
    ...
    if (req.files[type.toString()] && req.files[type.toString()][0]) await move(req.files[type.toString()][0].path, path.join(importDir, `${type.toString()}.csv`));
...
router.post("/profiles/:profile/accounting",
    upload.fields([{name: "accounting", maxCount: 1}, {name: "zipFile", maxCount: 1}]),
    schema.validate(importAccountingSchema),
    acl("profile-accounting", "write"),
    async (req, res, _) => {

        // When file missing throw error immediately
        if (!req.files || (!req.files["accounting"] && !req.files["zipFile"])) return res.status(400).send("Missing data file or zip file");
        if (isNaN(req.body.year)) return res.status(400).send("Invalid year value");
        ...
        if (req.files["zipFile"] && req.files["zipFile"][0]) {
            ...
            if (req.files["accounting"] && req.files["accounting"][0]) await move(req.files["accounting"][0].path, path.join(importDir, "accounting.csv"));
        }
