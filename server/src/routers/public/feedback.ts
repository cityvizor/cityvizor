...
router.post("/", schema.validate({body: feedbackSchema}), async (req, res, next) => {
...
router.post("/requestcity", schema.validate({body: requestCitySchema}), async (req, res, next) => {
...
function sendToProductboard(type: string, content: string) {
    axios.post("https://api.productboard.com/notes", {
        title: `Cityvizor - ${type}`,
        content: content,
        customer_email: "landing@cityvizor.cz"
    }, {
        headers: {"Authorization": `Bearer ${environment.keys.productboard.token}`}
    }).catch(err => {
        console.log(err)
    })
}