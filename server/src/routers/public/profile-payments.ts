...
router.get("/:year/csv", async (req, res, _) => {
    ...
    const payments = await db<PaymentRecord>("payments").where('profile_id', 'req.params.profile').andWhere('year', req.params.year);
    const events = await db<EventRecord>("events").where('profile_id', 'req.params.profile').andWhere('year', req.params.year);
    ...
    const header = ['profileId', 'year', 'paragraph', 'item', 'unit', 'event', 'eventName', 'date', 'amount', 'counterpartyId', 'counterpartyName', 'description'];
