...
router.get("/", acl("options:read"), async (req: Request, res: Response) => {
  try {
    const data =
      await db<SectionRecord>("app.sections").orderBy("orderOnLanding");
...
router.post("/", acl("options:write"), async (req: Request, res: Response) => {
  try {
    ...
    const [id] = await db("app.sections").insert(body, ["sectionId"]);
...
router.put(
  ...
  async (req: Request<{ id: string }>, res: Response) => {
    try {
      ...
      await db("app.sections").where("sectionId", req.params.id).update(body);
...
router.delete(
  ...
  async (req: Request<{ id: string }>, res: Response) => {
    try {
      await db("app.sections").where({ sectionId: req.params.id }).delete();
