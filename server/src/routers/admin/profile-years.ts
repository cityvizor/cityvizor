...
router.get("/", acl("profile-years", "list"), async (req, res, next) => {
  ...
  const years = await db<YearRecordWithImportStatus[]>("app.years AS y")
    .select(db.raw("distinct on (??) y.*, ??, GREATEST(??) as status_time", [["y.profileId", "y.year"], "i.status", ["i.created", "i.started", "i.finished"]]))
    .leftJoin("app.imports AS i", { "i.profileId": "y.profileId", "i.year": "y.year" })
    .where({ "y.profileId": req.params.profile })
    .orderBy([{ column: "y.profileId", order: "asc" }, { column: "y.year", order: "asc" }, { column: "i.id", order: "desc" }]);
