...
router.get('/', acl('profile-years', 'list'), async (req, res, _) => {
...
router.put('/:year', acl('profile-years', 'write'), async (req, res, _) => {
...
router.patch('/:year', acl('profile-years', 'write'), async (req, res, _) => {
...
router.delete('/:year', acl('profile-years', 'write'), async (req, res, _) => {
  await db<YearRecord>('app.years')
    .where('profile_id', req.params.profile)
    .andWhere('year', Number(req.params.year))
    .delete();
