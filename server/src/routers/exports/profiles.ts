...
import { ProfileRecord, AccountingRecord, PaymentRecord, EventRecord } from '../../schema/database';

import { exportQuery, exportArray } from './tools/export';
...
router.get("/:profile/years", async (req, res, next) => {
...
router.get("/:profile/accounting/:year", async (req, res, next) => {
...
router.get("/:profile/events/:year", async (req, res, next) => {
...
router.get("/:profile/payments/:year", async (req, res, next) => {

  const payments = db<PaymentRecord>("payments")
    .where({ profile_id: req.params.profile, year: req.params.year });
