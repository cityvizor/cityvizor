...
async function main() {
  for (const codelist of data) {
    await db('data.codelists')
      .where({codelist: codelist.name})
      .delete()
      .then(() => console.log('Cleared ' + codelist.name));
    await db('data.codelists')
      .insert(codelist.data.map(item => ({codelist: codelist.name, ...item})))
      .then(() => console.log('Populated ' + codelist.name));
  }
}
