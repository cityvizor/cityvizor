...
const osnovaJson = await parser.parseStringPromise(fs.readFileSync(process.argv[2]))
const entries = osnovaJson["ciselnik_UCTOSNOVA_export_20210406"]["row"].filter(row => {
        // Only valid entries
        return parseDate(row["start_date"]) <= Date.now() && parseDate(row["end_date"]) > Date.now()
    }).filter(row => {
...
const groups = [
    ...
    [68, "68x"],
    [69, "Převodové účty"],
].map(([id, name]) => {return {codelist: id.toString().startsWith("5") ? "pbo-su-exp-groups" : "pbo-su-inc-groups", id: id, name: name}})
