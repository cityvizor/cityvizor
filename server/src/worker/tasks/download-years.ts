...
export const TaskDownloadYears: CronTask = {
  ...
  exec: async () => {
  const years = await (await db<YearRecord>('app.years').whereNotNull('importUrl'))
  for (const year of years) {
    if (!year.importPeriodMinutes || !year.importUrl) continue
    const lastImport = await db<ImportRecord>('app.imports').first()
        .where('profileId', '=', year.profileId)
        .where('year', '=', year.year)
        .orderBy('created', 'desc')
    if (!lastImport || lastImport.created < new Date(Date.now() - 1000 * 60 * year.importPeriodMinutes)) {
      console.log(`Downloading new data for year ${year.year}, profile: ${year.profileId}`)
      const importDir = await Import.createImportDir();
      await axios.get(year.importUrl, {responseType: 'stream'}).then(async r => {
