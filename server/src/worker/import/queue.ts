...
export async function checkImportQueue() {
  ...
  if (runningJob) {
    ...
    // clear the old job with timeout
      ...
      const importDir = path.resolve(config.storage.imports, "import_" + runningJob.id)
  ...
  console.log(`[WORKER] ${DateTime.local().toJSDate()} Found a new job, starting import.`);
  ...
  try {
    ...
    const accountingFile = dirFiles.filter(file => file.match(/^.*accounting.*\.csv/i))[0];
    ...
    const files: Importer.OptionsFiles = {
      ...
      accountingFile: accountingFile ? path.join(importDir, accountingFile) : null
    };

    Object.keys(files).filter(k => files[k]).forEach(k => logger.log(`Found ${k} for import.`))
    ...
    if (!currentJob.append) {
      if (paymentsFile) { 
        await trx("data.payments").where({ profileId: currentJob.profileId, year: currentJob.year }).delete() 
        logger.log("Deleted previous payment records from the DB.")
      }
      if (eventsFile) { 
        await trx("data.events").where({ profileId: currentJob.profileId, year: currentJob.year }).delete() 
        logger.log("Deleted previous event records from the DB.")
      }
      if (accountingFile) { 
        await trx("data.accounting").where({ profileId: currentJob.profileId, year: currentJob.year }).delete() 
        logger.log("Deleted previous accounting records from the DB.")
      }
      if (dataFile) { 
        await trx("data.payments").where({ profileId: currentJob.profileId, year: currentJob.year }).delete() 
        await trx("data.accounting").where({ profileId: currentJob.profileId, year: currentJob.year }).delete() 
        logger.log("Deleted previous payment and accounting records from the DB.")
      }
    }
