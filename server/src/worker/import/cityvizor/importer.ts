...
import {createCityvizorParser, createCityvizorTransformer, CityvizorFileType} from "./parser"
...
export async function importCityvizor(options: Import.Options) {
    ...
    if (!options.append) {
        ...
        if (eventsFile) {
            await options.transaction('data.events')
                .where({profileId: options.profileId, year: options.year})
                .delete();
    ...
    for (const file_type of [
            [dataFile, CityvizorFileType.DATA], 
            [eventsFile, CityvizorFileType.EVENTS],
            [paymentsFile, CityvizorFileType.PAYMENTS], 
            [accountingFile, CityvizorFileType.ACCOUNTING]].filter(([f, _]) => f) as [string, CityvizorFileType][]) {
        const [file, type] = file_type
