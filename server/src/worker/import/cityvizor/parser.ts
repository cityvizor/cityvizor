...
export function createCityvizorParser(type: CityvizorFileType): csvparse.Parser {
  let headers: string[] = [];
  switch (type) {
...
export function createCityvizorTransformer(file: CityvizorFileType, options: Import.Options): Transform {
  switch (file) {
    case CityvizorFileType.ACCOUNTING:
    case CityvizorFileType.DATA:
    case CityvizorFileType.PAYMENTS:
...
function createAccountingRecord(row: {}, options: Import.Options): AccountingRecord {
    return ['type', 'paragraph', 'item', 'event', 'unit', 'amount'].reduce((acc, c) => acc[c] = row[c],
      {
        profileId: options.profileId,
        year: options.year,
      } as AccountingRecord
