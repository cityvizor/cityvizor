...
import { db } from "../db";
import { ProfileRecord } from 'src/schema';
...
router.get("/:profile", acl("profiles", "read"), async (req, res) => {
  ...
  const profile = await db<ProfileRecord>("profiles")
    .modify(function(){
      ...
      if(!isNaN(Number(req.params.profile))) this.orWhere({ url: Number(req.params.profile) })
    })    
    .first();
...
router.post("/", acl("profiles", "write"), async (req, res) => {
  ...
  const id = await db<ProfileRecord>("profiles")
    .insert(req.body, ["id"])
    .then(result => result[0].id);

  res.location(config.apiRoot + "/profiles/" + id);

  res.sendStatus(201);
});

router.put("/:profile", acl("profiles", "write"), function (req, res) {
